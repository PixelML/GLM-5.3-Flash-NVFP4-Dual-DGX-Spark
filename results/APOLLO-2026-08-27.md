# PixelML Apollo validation — 2026-08-27
...
- API: authenticated with `VLLM_API_KEY`; both the direct Tailscale route and
  public Funnel returned HTTP 401 without a key.
...
- Fresh ephemeral OpenCodex runs returned `GLM_LOCAL_OK` through
  `cliproxy/apollo-glm-5.3-flash` and `GLM_PUBLIC_OK` through
...
The head container started at `05:36:38Z`; the authenticated API became ready
at `05:51:56Z`, for a 15 minute 18 second first start. Rank-zero model loading
reported 91.23 GiB and 676.17 seconds; engine profile/KV/warm-up took another
133.79 seconds, followed by multimodal API warm-up. JIT and FlashInfer caches
persist in a Docker volume, so subsequent starts reuse compiled artifacts.
