...
> PixelML vendors the audited MiaAI recipe at upstream commit
> `bd7f55edff9e37b41e1d32e2cf37054fe66d1e58` and adds Apollo's exact topology,
> secret-file API authentication, comparable concurrency/prefill tests, and an
> independent dated receipt. The engine patches and original measurements are
> MiaAI's work; Brandon M. Music created the EXL3/TR3 checkpoint.
...
### PixelML Apollo reproduction

Apollo's two-Spark direct-CX7 profile (`apollo.env.example`) measured 66.30
decode tok/s for the five-run C1 structured protocol and 154.86 aggregate
tok/s across four structured streams. Code-only reached 49.60 C1 stream tok/s;
planning-heavy coding reached 27.50. The allocated KV pool was 942,857 tokens
at `GPU_MEM_UTIL=0.88`; a 299,527-token request, concurrent required-argument
tool calls, image, and video all passed. Full methodology and raw JSON:
[`APOLLO-2026-08-28-EXL3-DFLASH2.md`](../results/APOLLO-2026-08-28-EXL3-DFLASH2.md).
...
| `VLLM_API_KEY_FILE` | *(unset)* | mode-0600 host file; injects `VLLM_API_KEY` without writing or logging the credential |
...
| `GPU_MEM_UTIL` | `0.87` | GB10 UMA budget (DFlash2 + vision; upstream pool 982,612 tokens). Apollo needed 0.88 and allocated 942,857 |
...
| `tests/smoke_gate.py` | authenticated exact-response, code, and repetition-loop gate |
