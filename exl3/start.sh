...
# Keep the API credential out of .env and the host command line. The head
# container receives it through its environment; vLLM reads VLLM_API_KEY
# directly, so neither the generated command nor the startup log contains it.
VLLM_API_KEY_FILE="${VLLM_API_KEY_FILE:-}"
if [ -n "$VLLM_API_KEY_FILE" ]; then
    [ -r "$VLLM_API_KEY_FILE" ] || {
        echo "ERROR: VLLM_API_KEY_FILE is not readable" >&2
        exit 1
    }
    VLLM_API_KEY="$(<"$VLLM_API_KEY_FILE")"
fi
