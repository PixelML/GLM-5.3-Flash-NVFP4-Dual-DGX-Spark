#!/usr/bin/env python3
"""Streaming latency/throughput benchmark for the Apollo GLM DFlash2 endpoint."""
...
def run_one(url: str, api_key: str, prompt: str, max_tokens: int) -> dict:
    payload = {
    ...
    for concurrency in [int(x) for x in args.levels.split(",")]:
        ...
        for round_index in range(args.rounds):
            prompts = [
                ...
                + PROMPTS[(round_index * concurrency + i) % len(PROMPTS)]
            ...
            with concurrent.futures.ThreadPoolExecutor(concurrency) as executor:
                futures = [
                    executor.submit(
                        run_one, args.url, api_key, prompt, args.max_tokens
                    )
